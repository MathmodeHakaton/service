...
ru_filtered = ruonia[(ruonia["date"] >= d_from) &
                     (ruonia["date"] <= d_to)].copy()

window = min(260, max(30, len(ru_filtered) // 4))
ru_filtered["mad_score"] = mad_normalize(
    ru_filtered["rate_spread"], window=window)
