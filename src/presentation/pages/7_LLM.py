...
if prompt:
    if not creds_ok:
        ...
        retrieved = retrieve(search_query, chunks, k=top_k, prev_user_query=prev_q)
        ...
        with st.chat_message("assistant"):
            ...
            with st.expander("🩺 Debug: что реально ушло в LLM"):
                if search_query != prompt:
                    st.markdown(f"**Query rewrite:** `{prompt}` → `{search_query}`")
                st.markdown(f"**Извлечено чанков:** {len(retrieved)} / запрошено {top_k}")
                ...
                for i, c in enumerate(retrieved, 1):
                    ...
                    if len(preview) > 240:
                        preview = preview[:240] + "…"
                    st.write(f"{i}. **{c.title}**  ·  *{c.kind}*  ·  tags: `{tags}`")
                    st.caption(preview)
