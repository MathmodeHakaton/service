...
st.caption("Источник: Банк России — таблица дефицита/профицита ликвидности (с 2014)")
...
df5 = df5[(df5["date"] >= pd.Timestamp(d_from)) & (df5["date"] <= pd.Timestamp(d_to))].copy()
...
st.caption("Выше нуля = профицит (деньги с избытком). Ниже нуля = дефицит (банки занимают у ЦБ).")
