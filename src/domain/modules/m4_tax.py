...
class M4Tax(BaseModule):
    ...
    def __init__(self):
        super().__init__(name="M4_TAX", weight=0.0)  # мультипликатор, не аддитивный
    ...
    def compute_series(self, dates: pd.Series, tax_df: pd.DataFrame) -> pd.DataFrame:
        ...
        result["Tax_Week_Flag"] = result["date"].apply(
            lambda d: int(any(abs((d - kd).days) <= TAX_WEEK_WINDOW for kd in q_dates))
        )
    ...
    def _flags_for_date(self, d: pd.Timestamp, tax_df: pd.DataFrame) -> dict:
        ...
        eoq = int(d.month in [3, 6, 9, 12] and (d + pd.offsets.MonthEnd(0) - d).days < EOM_WINDOW)

        if eoq:   sf = SF_EOQ
        elif eom: sf = SF_EOM
        elif tw:  sf = SF_TAX_WEEK
        else:     sf = SF_NORMAL
    ...
    def _seasonal_factor(self, row) -> float:
        if row["End_of_Quarter_Flag"]: return SF_EOQ
        if row["End_of_Month_Flag"]:   return SF_EOM
        if row["Tax_Week_Flag"]:       return SF_TAX_WEEK
        return SF_NORMAL
