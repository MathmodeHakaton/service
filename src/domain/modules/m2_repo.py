...
class M2Repo(BaseModule):
    ...
    def _calculate_full(self, repo_full: pd.DataFrame, keyrate_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["keyrate"] = df["date"].map(lambda d: kr.asof(
            d) if d >= kr.index.min() else np.nan)
        df["rate_spread"] = df["rate_wavg"] - df["keyrate"]

        df["MAD_score_rate_spread"] = mad_normalize(
            df["rate_spread"],  window=MAD_WINDOW)
    ...
    def _merge_bliquidity_signals(self, df: pd.DataFrame, bliq: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        want = ["date", "auction_repo_bln",
                "standing_repo_bln", "standing_secured_credit_bln"]
    ...
    def _calculate(self, repo_df, keyrate_df, params_df) -> pd.DataFrame:
        ...
        if not params_df.empty:
            p7 = params_df[params_df["term_days"] ==
                           PRIMARY_TERM][["date", "limit_bln"]]
        ...
        df["MAD_score_rate_spread"] = mad_normalize(
            df["rate_spread"],  window=MAD_WINDOW)
        df["MAD_score_cover"] = mad_normalize(
            df["utilization"],  window=MAD_WINDOW)
